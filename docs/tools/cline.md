...
- Base URL 应填写服务商端点，不要直接使用 OpenAI 默认地址。
- 模型名请以服务商提供的模型列表为准。
- 修改设置后，请确认已经点击 `Save` 或 `Verify`。
